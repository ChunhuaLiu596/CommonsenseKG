...
def greedy_alignment(embed1, embed2, top_k, nums_threads, metric, normalize, csls_k, accurate):
    ...
    num = sim_mat.shape[0]
    rank_candidates_num = sim_mat.shape[1]
    if nums_threads > 1:
        hits = [0] * len(top_k)
        mr, mrr = 0, 0
        alignment_rest = list()
    ...
    del sim_mat
    gc.collect()
    return  mr, mrr, hits, alignment_rest
...
def calculate_rank(idx, sim_mat, top_k, accurate, total_num):
    ...
    for i in range(len(idx)):
        ...
        if accurate:
            ...
            #if not np.array_equal(rank, rank_accurate):
            #    for k in top_k:
            #        print("k: {}, inaccurate vs accurate: {} : {}".format(k, rank[k],rank_accurate[k]))
        rank_score = [np.round(sim_mat[i,j], 4) for j in rank[0:10]] 
